...
describe("Test updateById", ()=>{
    ...
    it("should return error object if query id is not matched", async ()=>{
        const result = await updateById({query: {_id: "rAnD0MstR!n9"}, update: {name: "newname1"}} as any) as any;
        expect(result).toBeDefined();
        expect(result.message).toBeUndefined();
    })

    it("should return status if no error is catched", async()=>{
        const result = await updateById({query: {_id: "65276274f4d1aec85d6fb092"}, update: {name: "newname2"}} as any) as Acknowledged;
        expect(result).toBeDefined();
